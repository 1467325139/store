...
def depositmoney():
    ...
    if account in bank:
        ...
        -------------------------------
        '''
        print(info  % (account,deposit,bank[account]["money"],bank_name,now_time))
        return True
    else:
        print("账号不存在")
...
def takemoney():
    ...
    if account in bank:
        if password == bank[account]["password"]:
            ...
            if retrieve <= bank[account]["money"]:
                bank[account]["money"] -= retrieve
                ...
                info = '''
                        ...
                        -------------------------------
                        '''
                print(info % (account,retrieve, bank[account]["money"], bank_name,now_time))
            else:
                print("钱不够")
                return 3
        else:
...
def transfermoney():
    ...
    password = input("请输入密码：")
    if account_1 in bank:
        if password == bank[account_1]["password"]:
            account_2 = int(input("请输入要转账的账号："))
            if account_2 in bank:
                movemoney = int(input("请输入转账金额："))
                if movemoney <= bank[account_1]["money"]:
                    bank[account_1]["money"] -= movemoney
                    bank[account_2]["money"] += movemoney
                    print("转账成功,")
                    info = '''
                            -----------------------------
                                       转账凭证
                             ...
                             -------------------------------
                            '''
                    print(info % (account_1,account_2,movemoney,bank[account_1]["money"], bank_name,now_time))
                    return 0
                else:
                    print("钱不够")
                    return 3
...
def step_bank_transfermoney():
    ...
    from INDUSTRIAL_AND_COMMERCIAL_BANK import IC_bank
    if account_1 in bank and account_2 in bank:
        transfermoney()
    elif account_1 in bank and account_2 in IC_bank:
        password = input("请输入转出账户密码：")
        if password == bank[account_1]["password"]:
            movemoney = int(input("请输入转出金额："))
            n = cost(movemoney)
            if movemoney <= (int(bank[account_1]["money"])-n):
                bank[account_1]["money"] = int(bank[account_1]["money"]) - movemoney - n
                IC_bank[account_2]["money"] = int(IC_bank[account_2]["money"]) + movemoney
                ...
                info = '''
                    -----------------------------
                     ...
                     -------------------------------
                                            '''
                print(info % (account_1, account_2, movemoney, n,bank[account_1]["money"],bank_name, now_time))
            else:
                print("账户余额不足！！！")
        else:
            print("密码不正确！！")
    ...
    if account in bank:
        if password == bank[account]["password"]:
            ...
            info = '''
                   ...
                   -------------------------------
                   '''
            print(info % (account, bank[account]["username"],bank[account]["password"],bank[account]["country"], bank[account]["province"], bank[account]["street"], bank[account]["door"], bank[account]["money"], bank_name,now_time))
        else:
            print("密码输入错误")
    else:
        print("账号不存在")
